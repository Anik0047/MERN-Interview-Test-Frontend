...
const CanvasDrawing = () => {
  ...
  const startDrawing = (e) => {
    ...
    if (tool === "text") {
      ...
      setTextPosition({
        x: offsetX + canvasRect.left,
        y: offsetY + canvasRect.top,
      });
    ...
    if (tool === "brush" || tool === "eraser") {
      ...
      setElements((prevState) => [
        ...prevState,
        {
          ...
          color: tool === "brush" ? selectedColor : "#fff",
          brushWidth,
        },
      ]);
    } else {
      ...
      setElements((prevState) => [
        ...prevState,
        { element, offsetX, offsetY, type: tool },
      ]);
    }
  };
  ...
  const draw = (e) => {
    ...
    if (tool === "brush" || tool === "eraser") {
      ...
      if (tool === "eraser") {
        setTextInputs((prevTextInputs) =>
          prevTextInputs.filter((textInput) => {
            ...
            const withinXRange =
              offsetX > textInput.x && offsetX < textInput.x + textWidth;
            const withinYRange =
              offsetY > textInput.y - textHeight && offsetY < textInput.y;
      ...
      if (tool === "rectangle") {
        ...
        lastElement.element = rough
          .canvas(canvasRef.current)
          .circle(lastElement.offsetX, lastElement.offsetY, radius * 2, {
            stroke: selectedColor,
            fill: fillColor ? selectedColor : "transparent",
            strokeWidth: brushWidth,
          });
        ...
        lastElement.element = rough.canvas(canvasRef.current).polygon(
          [
            ...
            [offsetX, offsetY],
            [lastElement.offsetX * 2 - offsetX, offsetY],
          ],
          {
            stroke: selectedColor,
            fill: fillColor ? selectedColor : "transparent",
  ...
  const handleTextSubmit = (e) => {
    ...
    setTextInputs((prevState) => [
      ...
      {
        text: inputText,
        x: textPosition.x - canvasRect.left,
        y: textPosition.y - canvasRect.top,
      },
    ]);
    setInputText("");
  ...
  const saveImage = async () => {
    ...
    try {
      const response = await fetch(
        "https://whiteboard-backend-sooty.vercel.app/save-image",
        {
          method: "POST",
          headers: {
      ...
      console.log("REs in frint", response);
        ...
        <section className="tools-board">
          <div className="row">
            ...
            <ul className="options">
              ...
              <li
                className={`option tool ${tool === "circle" ? "active" : ""}`}
                onClick={() => setTool("circle")}
              >
                <span>Circle</span>
              </li>
              <li
                className={`option tool ${tool === "triangle" ? "active" : ""}`}
                onClick={() => setTool("triangle")}
              >
                <span>Triangle</span>
              </li>
              <li
                className={`option tool ${tool === "brush" ? "active" : ""}`}
                onClick={() => setTool("brush")}
              >
                <span>Brush</span>
              </li>
              <li
                className={`option tool ${tool === "eraser" ? "active" : ""}`}
                onClick={() => setTool("eraser")}
              >
                <span>Eraser</span>
              </li>
              <li
                className={`option tool ${tool === "text" ? "active" : ""}`}
